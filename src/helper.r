...
update_nowcast_web_data <- function(unctad_nowcast_web_directory, latest_database, target_period) {
  ...
  for (target_variable in c("x_world", "x_servs_world", "x_vol_world2")) {
    plot_data <- read_csv(paste0("output/", latest_database, "_", target_variable, "_plot_df.csv"))
    data <- data %>%
      filter(!((target_variable == !!target_variable) & (target_period == !!target_period))) # getting rid of old data for this period
    new_data <- plot_data %>% 
      mutate(target=target_variable, target_period=target_period)
    data <- rbind(data, new_data)
  }
  write_csv(paste0(unctad_nowcast_web_directory, "nowcasts/data/data.csv"))
}